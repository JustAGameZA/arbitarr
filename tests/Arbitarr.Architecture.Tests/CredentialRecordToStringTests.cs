...
public class CredentialRecordToStringTests
{
    ...
    ///
    /// <para>A record is secret-bearing when its name ends in <c>Credential</c>, OR it declares an
    /// instance property whose name is exactly one of <see cref="SecretPropertyNames"/>. The
    /// name-suffix arm catches a credential type whose member is called something this list does not
    /// know; the property arm catches a secret-carrying type that is not named like one — which is
    /// most of them.</para>
    /// </summary>
        ...
        ScanForSecretBearingRecords()
    {
        ...
        foreach (var assemblyName in ScannedAssemblies)
        {
            ...
            foreach (var type in module.GetTypes())
            {
                ...
                var namedCredential = type.Name.EndsWith("Credential", StringComparison.Ordinal);
                var secretProperties = type.Properties
                    .Where(p => p.HasThis && SecretPropertyNames.Contains(p.Name, StringComparer.Ordinal))
                    .Select(p => p.Name)
                    .ToArray();
                ...
                var reason = namedCredential && secretProperties.Length > 0
                    ? $"name ends in 'Credential'; properties {string.Join(", ", secretProperties)}"
                    : namedCredential
                        ? "name ends in 'Credential'"
