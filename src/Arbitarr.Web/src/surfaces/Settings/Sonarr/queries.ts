...
import type { ArrConfig, ArrTestResult, UpdateArrConfigRequest } from './types';

const SONARR_KEY = ['admin', 'arr', 'sonarr'];
const SONARR_ROUTE = '/api/admin/arr/sonarr';

/**
 * GET /api/admin/arr/sonarr.
 *
 ...
 */
export function useSonarrConfigQuery() {
  return useQuery({
    queryKey: SONARR_KEY,
    queryFn: () => apiFetch<ArrConfig>(SONARR_ROUTE),
  });
}

/**
 * PUT /api/admin/arr/sonarr.
 *
 ...
 * `Sources/queries.ts` and `Notifications/queries.ts` and deliberately DIFFERS
 * from `Ai/queries.ts`, whose body carries nothing secret (see bead arb-689 for
 * why that asymmetry is intentional). The caller also `reset()`s on settle, which
 * drops it immediately rather than at gc.
 *
 ...
 */
export function useUpdateSonarrConfigMutation() {
  const client = useQueryClient();
  return useMutation({
    gcTime: 0,
    mutationFn: (request: UpdateArrConfigRequest) =>
      apiFetch<ArrConfig>(SONARR_ROUTE, {
        method: 'PUT',
        body: JSON.stringify(request),
      }),
    onSuccess: () => client.invalidateQueries({ queryKey: SONARR_KEY }),
  });
}
 ...
 * Sources surface never offers. `DELETE /api/admin/arr/sonarr` unconfigures the
...
/**
 * POST /api/admin/arr/sonarr/test — the connectivity probe.
 *
 ...
 * asked for.
 */
export function useTestSonarrMutation() {
  return useMutation({
    mutationFn: () => apiFetch<ArrTestResult>(`${SONARR_ROUTE}/test`, { method: 'POST' }),
  });
}
