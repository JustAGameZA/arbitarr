...
/// collected in ONE place (arb-fxw) so the writers and <see cref="StagingSweepService"/> cannot
...
{
    /// <summary>Prefix of the archive <see cref="AdminBackupEndpoints"/>' download route builds via <see cref="BackupService"/>.</summary>
    public const string DownloadPrefix = "arbitarr-download-";

    /// <summary>Prefix of the spooled upload <see cref="AdminBackupEndpoints"/>' restore route stages before validation.</summary>
    ...
    /// <see cref="StagingSweepService"/> to enumerate. Adding a fifth writer means adding its
