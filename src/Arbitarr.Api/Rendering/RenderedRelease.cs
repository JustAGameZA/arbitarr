...
///
/// <para>A member's guid now RESOLVES at the download proxy (arb-vlsu); nothing yet OFFERS one — no
/// rendered surface emits a member guid to Sonarr/Radarr and the proxy does not fall back
/// automatically. Retaining the members is what makes resolution possible, and <c>SearchEndpoint</c>
/// now registers every group member — not only the representative — in the release lookup (both the
/// in-memory tier and the durable <c>IReleaseLookupStore</c>), each under its own
/// <see cref="ProxyGuid"/>, so a member resolves at the download proxy exactly as its representative
/// does.</para>
///
