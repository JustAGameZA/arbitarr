...
**Amended 2026-09-17.** The "seven" above is stale — the count has drifted as clients were added since this ADR was written. The property that matters is that `AllowAutoRedirect = false` is set on every typed HTTP client registration in the composition root (`src/Arbitarr.Host/Program.cs`), which is what SEC-M1 actually requires, not any particular count. `grep -c "AllowAutoRedirect = false" src/Arbitarr.Host/Program.cs` measures the current registration count directly, and equals the number of registrations only while each one sets the property on its own line. `HttpClientRedirectSweepTests.Every_registered_HttpClient_refuses_automatic_redirects` (`Arbitarr.Integration.Tests`) now asserts the property across every registration, discovered at runtime against the real composed host rather than hardcoded (arb-673k); the source-text guard `ProgramOllamaHttpClientTests` (see the Consequences section) remains as a second, independent pin on the one registration it covers.
...
The download request includes a query string carrying the NZBHydra2 API key. If the redirect were followed, the `Location` header would include that key, and following it would require a second outbound request — a second call to `ReadApiKeyForUpstreamRequestAsync`. The redaction and one-caller mechanisms this would put at risk are documented once, in [CLAUDE.md §1](../../CLAUDE.md#1-secrets-three-mechanisms-that-must-survive-refactoring); the property that every registration refuses automatic redirects is enforced today by a runtime sweep against the real composed host, `HttpClientRedirectSweepTests.Every_registered_HttpClient_refuses_automatic_redirects` (`Arbitarr.Integration.Tests`), with the pre-existing source-text guard `ProgramOllamaHttpClientTests` (`Arbitarr.Host.Tests`) remaining as a second, independent pin on the one registration it covers. The sweep discovers client names via `IHttpClientFactory`'s own registration surfaces rather than a hardcoded roster, so it does not see a client registered with the bare, zero-configuration `AddHttpClient(name)` overload and no builder configuration; no client in this codebase is registered that way today.
