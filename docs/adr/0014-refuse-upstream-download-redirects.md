...
The download request includes a query string carrying the NZBHydra2 API key. If the redirect were followed, the `Location` header would include that key. The HttpClientFactory logging handler logs request URIs but collapses query strings to `?*` by default (a .NET framework behavior pinned by `DisableUriRedactionSwitchTests` in CLAUDE.md §1); of the eight `AllowAutoRedirect = false` registrations, only the Ollama one is test-pinned (`ProgramOllamaHttpClientTests`). Following the redirect would require a second outbound request — a second call to `ReadApiKeyForUpstreamRequestAsync`, which violates the one-caller rule (CLAUDE.md §1) and would be a second site where credentials could leak. The `LogMessageCleanser` scrubs credentials in query strings but not in URL paths; a key in a redirect path segment (indexer-dependent) is covered by neither the redaction nor the cleanser.
